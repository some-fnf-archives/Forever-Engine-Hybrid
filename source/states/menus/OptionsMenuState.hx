...
import funkin.menu.Checkmark;
import funkin.menu.Selector;
import states.subState.OptionsSubstate;
...
class OptionsMenuState extends MusicBeatState
{
			...
			if (Init.gameSettings.get(letter.text) != null)
			{
				switch (Init.gameSettings.get(letter.text)[1])
				{
					case Init.SettingTypes.Checkmark:
						// checkmark
						var checkmark = new Checkmark(0, 0);
	...
	function updateCheckmark(checkmark:ForeverSprite, animation:Bool)
	{
		if (checkmark != null)
